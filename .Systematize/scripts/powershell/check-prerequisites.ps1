...
if (-not (Test-Path $paths.FEATURE_DIR -PathType Container)) {
    ...
    Write-Output "Run /syskit.systematize first to create the feature structure."
    exit 1
}
...
if (Test-Path $paths.AGENTS_MD) { $docs += 'AGENTS.md' }
...
if ($Json) {
    ...
    Test-FileExists -Path $paths.AGENTS_MD -Description 'AGENTS.md' | Out-Null
