...
3. **Understand user request**: Combine `$ARGUMENTS` + clarifying answers:
   ...
   - Infer any missing context from sys/plan/tasks (do NOT hallucinate)

4. **Load feature context**: Read from FEATURE_DIR:
   - sys.md: Feature requirements and scope
...
5. **Generate checklist** - Create "Unit Tests for Requirements":
   ...
   **HOW TO WRITE CHECKLIST ITEMS - "Unit Tests for English"**:
   ...
   - "Does the sys define visual hierarchy for competing UI elements?" [Clarity]
...
**Important**: Each `/syskit.checklist` command invocation uses a short, descriptive checklist filename and either creates a new file or appends to an existing one. This allows:
